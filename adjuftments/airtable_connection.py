...
from adjuftments.config import AirtableColumnMapping, AirtableConfig
from adjuftments.schema import (BudgetsTable, CategoriesTable,
...
class Airtable(AirtablePythonWrapper):
    ...
    def process_airtable_response(table: str, response: dict) -> dict:
        ...
        elif table.lower() == "miscellaneous":
            updated_response = Airtable._process_measure_value_response(airtable_response=response)
        elif table.lower() == "dashboard":
            updated_response = Airtable._process_dashboard_response(airtable_response=response)
        ...
        elif table.lower() == "accounts":
            updated_response = Airtable._process_accounts_response(airtable_response=response)
        else:
    ...
    def get_column_mapping_json(table: str, airtable_dict: dict) -> dict:
        ...
        if table == "expenses":
            ...
            for key, value in airtable_dict.items():
                updated_dict_key = AirtableColumnMapping.EXPENSES_REVERSE[key]
                # DO NOT UPDATE COMPUTED VALUES
                if key not in ["updated_at", "account_name"]:
    ...
    def _process_expense_response(airtable_response: dict) -> dict:
        ...
        return dict(
            ...
            date=expense_date,
            account=airtable_response["fields"].get("Account", [None])[0],
            account_name=airtable_response["fields"].get("AccountName", [None])[0],
    ...
    @staticmethod
    def _process_dashboard_response(airtable_response: dict) -> dict:
        """
        Prepare a flattened dict with all columns (using defaults if needed)
        from a generic Measure, Value table
        ...
        return dict(
            ...
            measure=airtable_response["fields"].get("Measure"),
            value=airtable_response["fields"].get("Value"),
            ordinal_position=airtable_response["fields"].get("Position"),
            created_at=airtable_response["createdTime"])

    @staticmethod
    def _process_accounts_response(airtable_response: dict) -> dict:
        """
        Prepare a flattened dict with all columns (using defaults if needed)
        from a the Accounts Table
        ...
        return dict(
            id=airtable_response["id"],
            name=airtable_response["fields"]["Name"],
            type=airtable_response["fields"]["Type"],
            description=airtable_response["fields"].get("Description"),
            balance=airtable_response["fields"].get("Balance", 0.00),
            starting_balance=airtable_response["fields"].get("Starting Balance", 0.00),
            default=airtable_response["fields"].get("Default", False),
            created_at=airtable_response["createdTime"])
